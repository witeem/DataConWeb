...
import { UpdateOutletApi } from "@/api/modules/outlet";
import { getLevelList, getTenantCategorys, getZoneList } from "../interface";
...
const UpdateForm = (props: any) => {
	...
	const showModal = (params: TableListItem) => {
		...
		if (params.leaseStartDate && params.leaseEndDate) {
			let leaseDate = [moment(params.leaseStartDate), moment(params.leaseEndDate)];
			form.setFieldValue("leaseDate", leaseDate);
		}
	};
	...
	const createBtn = async () => {
		try {
			...
			if (userReq && validates) {
				let res = await UpdateOutletApi(GetOutletReq(userReq));
				if (res.success) {
		...
		>
			<Form name="form_in_modal" layout="vertical" form={form} initialValues={{ modifier: "public" }}>
				...
				<Row>
					...
					<Col span={2}></Col>
					<Col span={11}>
						<Form.Item label="Zone" name="zone" rules={[{ required: true, message: "Please Selected the Zone" }]}>
							<Select
								showSearch
